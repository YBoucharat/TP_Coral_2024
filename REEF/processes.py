...
from tools import readfile, Volume
...
class SLFile:
    ...
    def initialize(self):
        ...
        dico = Dicos()
        # Reads SL file and transforms age in time
...
class UniformGrid1D:
    ...
    hmax = xs.variable(description="maximum water height for reef growth", attrs={'unit':'m'})
    ...
    def initialize(self, tmax):
        ...
        self.length = int((self.hmax + 900 + abs(self.u) * tmax) / self.slopi)+6000
...
class InitTopo:
    ...
    hmax = xs.foreign(UniformGrid1D, 'hmax')
    ...
    def initialize(self, tmax):
        ...
        if int(self.u*1e5) > 0:
            if self.slopi>=10e-2:
                ...
                shift = 400
            self.z = self.slopi * self.x - (self.hmax+abs(self.u*tmax)+shift)
        else:
            if self.slopi>=10e-2:
                shift = 1000
            else:
                shift = 500
            self.z = self.slopi * self.x -(shift+self.hmax)
...
class InitTopoTerr:
    ...
    def initialize(self, tmax):
        ...
        if self.u > 0:
            self.z = self.slopi * self.x - (self.hmax+abs(self.u*tmax)+500)
        else:
            self.z = self.slopi * self.x -(700+self.hmax)
...
class SedimClastics():
    ...
    dG = xs.foreign(Construction, 'dG')
    start_eros = xs.foreign(Erosion, 'start_eros')
    z = xs.foreign(ProfileZ, 'z')
    dE = xs.foreign(Erosion, 'dE')
    asl = xs.foreign(SeaLevel, 'asl')
