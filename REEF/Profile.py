...
#*********************************************************************
    ...
    def __init__(self, 
        ...
        self.time_ASL = ds.time.values.copy()
        self.ASL = ds.sealevel__asl.values.copy()
        ...
        if tmax == -1 or anim:
            ...
            self.itmax = np.argmax(ds.time.values == tmax)
            self.tmax=ds.time[self.itmax].values
            # Slicing dataset to tmax 
            if self.tmax == 0:
                self.ds = ds.isel(time=slice(0, 1), x=slice(None, None))
            else:
                self.ds = ds.isel(time=slice(None, self.itmax+1), x=slice(None, None))
        
        # Defines profile boundaries
        self.AxeBoundaries(xmin, xmax, ymin, ymax)
        # Slicing dataset to profile boundaries
        self.ds = self.ds.isel(time=slice(None, None), x=slice(self.xmin,self.xmax))
...
#         self.cmap = cmap
        ...
        self.ds['dU'] = xr.Variable(data = self.ds.vertical__u.values * self.ds.time.values, dims = ('time'))
        # Displaced profiles
        self.ds['z'] = xr.Variable(data = np.zeros(np.shape(self.ds.profile__z)), dims=('time', 'x'))        
        # Displaced final (with erosion) profiles
        self.ds['zf'] = xr.Variable(data = np.zeros(np.shape(self.ds.z)), dims=('time', 'x'))
        # Total sediment thickness to plot
        self.ds['sed'] = xr.Variable(data = np.zeros((len(self.ds.time), len(self.ds.init__x))), dims = ('time', 'x'))
        # Clastic sediments after erosion
        self.ds['clasts'] = xr.Variable(data = np.zeros((len(self.ds.time), len(self.ds.init__x))), dims = ('time', 'x'))
        ...
        for t in range(len(self.ds.time)):
            self.ds.zf[t] = self.ds.z[t:].min(axis = 0)
        ...
        for i in range(1, len(self.subds.time)):
        ...
        self.ax.plot(
            ...
            self.subds.zf.T.values, 
            color='black', alpha=0.2, lw=0.15, rasterized = True)
        
        # Plot clastics
        for t in range(len(self.ds.time)-1):
        ...
        self.ax.plot(self.ds.init__x, self.ds.zf[-1], color='black', lw=0.4)
        ...
        if hmax:
            ...
            print('xmax_hmax', xmax_hmax, 'riv', riv)
...
#*********************************************************************
    ...
    def SL_Colormap(self, cmap = cm.roma_r):
        ...
        self.ds['color'] = xr.Variable(
            data = np.zeros((int(self.time_ASL[-1]/self.dt)+1, 4)),
            dims = ('time', 'colo')
        )
        self.ds.color[:] = self.cmap(np.linspace(0, 1, int(self.time_ASL[-1]/self.dt)+1))
        
        # To run MIS based colormap
        if self.SL in ['Holocene', 'Waelbroeck2002', 'Waelbroeck2002b', 'Waelbroeck2002-137ky']:
            self.MIS_Colormap()
...
#*********************************************************************
    ...
    def AxeBoundaries(self, xmin, xmax, ymin, ymax):
        ...
        if xmin==0:
            self.xmin = int(self.ds.profile__xmin.values-100)
        else:
            self.xmin=int(xmin)
        if xmax==0:
            self.xmax = int(self.ds.profile__xmax.values+100)
        else:
            self.xmax = int(xmax)
        if ymin==0:
...
#*********************************************************************
    ...
    def PlotASL(self, 
        ...
        if loc == 'bottom':
            ...
            axSL= self.ax.inset_axes([1 - prop_width, 0., prop_width, prop_height])
        ...
        # Plots ASL
        axSL.scatter(
            self.time_ASL[::-1]/1000, 
            self.ASL,
            c=self.ds.color, s=s, marker='o') 
        ...
        axSL.set_xlim(0, self.time_ASL[-1]/1000+1)
...
#*********************************************************************
    ...
    def PlotData(self, namefile, color='black', lw=1, dx=0):
        ...
        print(np.argmax(self.ds.profile__z[-1].values>=z[0]))
        self.ax.plot(self.ds.init__x, self.ds.profile__z[-1])
